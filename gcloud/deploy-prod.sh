...
if [ "$TRAVIS_COMMIT_MESSAGE" == *"trigger build react"* ]; then
  echo "Building React Web App..."
  docker build -t eu.gcr.io/${GCP_PROJECT_ID}/${DOCKER_IMAGE}:$TRAVIS_COMMIT -f react-webapp/Dockerfile .
elif [ "$TRAVIS_COMMIT_MESSAGE" == *"trigger build node"* ]; then
  echo "Building NodeJS Service..."
  docker build -t eu.gcr.io/${GCP_PROJECT_ID}/${DOCKER_IMAGE}:$TRAVIS_COMMIT -f nodejs-service/Dockerfile .
fi
