...
### Addendum: what live device testing on production actually required

Static analysis and build success weren't enough - end-to-end testing against production (real device via wireless adb, real Google account) surfaced three additional real bugs/gaps, all now fixed except the last:

1. **`/webauthn/**` requires a session cookie, not just the JWT.** Verified against `spring-security-webauthn:7.0.4` source (`PublicKeyCredentialCreationOptionsFilter`): it checks `SecurityContextHolder` directly, populated only by that chain's own session-based login (`formLogin`/`oauth2Login`/`oneTimeTokenLogin`) - a Bearer JWT alone gets a hard, bodyless `400`. react-webapp only "works" because a browser that logged in via Google OAuth2 keeps that session cookie indefinitely; a purely JWT-authenticated client (Android's `/api/authenticate`, or a react user who never used Google login) hits the same wall. Fixed by switching Google login (`LoginActivity.performGoogleLogin()`) from a Chrome Custom Tab to an in-app WebView (`GoogleLoginWebViewActivity`), so the resulting `SESSIONID` cookie (invisible to OkHttp when held by Chrome) can be read via `android.webkit.CookieManager` and imported into `ApiClient`'s cookie jar.
2. **`ApiClient`'s `CookieJar.saveFromResponse()` silently overwrote the per-host cookie list on every response** - including with an empty list when a response had no `Set-Cookie` header at all (which OkHttp still calls `saveFromResponse` for). This wiped the just-imported `SESSIONID` on the very next request. Fixed to merge by cookie name instead of overwriting.
3. **Missing fresh CSRF token.** `webauthn/register/options`/`webauthn/authenticate/options` are CSRF-protected; matching react-webapp's `PasskeyEdit.js`/`Login.js`, both flows now call `AuthService.getCsrfToken()` immediately before the webauthn call rather than relying on a possibly stale/absent cached value.
4. **Not fixed - backend deploy needed:** even with a valid session, Google Play Services blocks passkey registration with `RP ID cannot be validated` until `https://<rpId>/.well-known/assetlinks.json` exists declaring `com.springboot.android`'s signing certificate. Added `authentication-service/src/main/resources/static/.well-known/assetlinks.json` with the debug keystore's SHA-256 fingerprint (`75:2D:05:B2:C2:30:14:4F:DF:DD:D6:B8:DC:21:65:CE:86:7E:45:7C:9A:5B:85:6E:AD:1F:1D:72:A2:3C:92:32`); a release build's fingerprint still needs adding before release passkey registration works. Requires deploying authentication-service - not yet verified against the live deployment.
