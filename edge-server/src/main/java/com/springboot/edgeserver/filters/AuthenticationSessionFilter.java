...
/**
 * Filter applied after authentication-service to set user in the session.
 */
@Slf4j
@AllArgsConstructor
@Component
public class AuthenticationSessionFilter implements GatewayFilter {
        ...
        return chain.filter(exchange)
                ...
                .flatMap(w -> w.getSession().flatMap(session -> {
                    ...
                    return springSessionWebSessionStore.retrieveSession(sessionId)
                            .flatMap(s -> {
                                session.getAttributes().putAll(s.getAttributes());
                                return session.save();
                            });
                })
                .then());
