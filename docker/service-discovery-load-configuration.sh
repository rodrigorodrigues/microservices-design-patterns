...
if ! curl -k --retry 5 --retry-connrefused --retry-delay 5 -H "Content-Type: application/json" -sf http://service-discovery:8500/v1/kv/config; then
    ...
    curl --request PUT --data-binary @spring-cloud-config/applicationDev.yml http://service-discovery:8500/v1/kv/config/week-menu-api/data
    ...
    curl --request PUT --data-binary @spring-cloud-config/applicationDev.yml http://service-discovery:8500/v1/kv/config/dotnet-service/dev
    curl --request PUT --data-binary @spring-cloud-config/applicationDev.yml http://service-discovery:8500/v1/kv/config/scala-address-service/dev
fi
